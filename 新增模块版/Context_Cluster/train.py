...
parser.add_argument('--warmup-lr', type=float, default=1e-9, metavar='LR',
                    help='warmup learning rate (default: 1e-6)')
parser.add_argument('--min-lr', type=float, default=1e-9, metavar='LR',
...
def main():
    ...
    args, args_text = _parse_args() # 解析命令行参数并返回对象
    
    if args.log_wandb: # 是否执行wandb
        if has_wandb:
...
def validate(model, loader, loss_fn, args, amp_autocast=suppress, log_suffix=''):
    ...
    with torch.no_grad():
        for batch_idx, (input, target) in enumerate(loader):
            ...
            correct_predictions += acc1.item() * output.size(0)
    ...
    avg_acc1 = correct_predictions / total_predictions
    avg_acc5 = top5_m.avg  # 使用AverageMeter对象中的平均值
    print('avg_acc1', avg_acc1)
    print('avg_acc5', avg_acc5)
    # metrics = OrderedDict([('loss', losses_m.avg), ('top1', top1_m.avg), ('top5', top5_m.avg)])
    metrics = OrderedDict([('loss', losses_m.avg), ('top1', avg_acc1), ('top5', avg_acc5)])
    return metrics
